...
var operators = []Operator{
	...
	},
	{
		...
		Doc:      "Stream schema/coverage summary: field, type, coverage, distinct_est, top_values, min, max, avg, p25, p50, p75, null_pct (RFC-002 §7.4).",
	...
	// sugar (mechanical desugar, RFC-002 §9.1)
	{
		...
		Syntax:     "top [n] <field> [by <fields>]",
		Grammar:    "'top' [number] field ['by' field_list]",
		DesugarsTo: "stats count() as count by <field>[, <fields>] | sort <fields>, -count | head <n> or per-group row_number filter",
		Doc:        "Top-N frequent values.",
		Examples:   []string{`top 10 uri`, `top 3 uri by service`},
	},
	{
		...
		Syntax:     "rare [n] <field> [by <fields>]",
		Grammar:    "'rare' [number] field ['by' field_list]",
		DesugarsTo: "stats count() as count by <field>[, <fields>] | sort <fields>, count | head <n> or per-group row_number filter",
