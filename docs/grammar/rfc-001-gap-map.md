...
| CLI `--show-rewritten` and TUI rewrite blocks | Deferred | `--no-lint` now disables server-side advisory lints and returned lints render on stderr; server query responses now include structured rewrite data, but CLI/TUI rendering flags are not wired yet. |
