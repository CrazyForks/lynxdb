...
func TestLexer_Keywords(t *testing.T) {
	input := `FROM where SEARCH stats eval sort head tail reverse timechart rex regex fields table dedup mvexpand expand by as and or xor not`
	lexer := NewLexer(input)
	tokens, err := lexer.Tokenize()
	if err != nil {
		t.Fatalf("Tokenize: %v", err)
	...
	expected := []TokenType{
		...
		TokenFields, TokenTable, TokenDedup, TokenMvexpand, TokenExpand, TokenBy, TokenAs,
