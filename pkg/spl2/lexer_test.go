...
func TestLexer_Keywords(t *testing.T) {
	input := `FROM where SEARCH stats eval sort head tail reverse timechart chart rex regex replace fieldformat fields table dedup xyseries untable mvexpand expand makeresults makemv mvcombine nomv by over as and or xor not`
	lexer := NewLexer(input)
	tokens, err := lexer.Tokenize()
	if err != nil {
		t.Fatalf("Tokenize: %v", err)
	...
	expected := []TokenType{
		...
		TokenFields, TokenTable, TokenDedup, TokenXyseries, TokenUntable, TokenMvexpand, TokenExpand, TokenMakeresults, TokenMakemv, TokenMvcombine, TokenNomv, TokenBy, TokenOver, TokenAs,
