...
| Eval functions `like`, `toint`, `todouble`, `tobool`, `isbool`, `typeof`, `log`, `exp`, `pow`, `pi`, variadic `max`, and variadic `min` compile and execute through VM opcodes and are present in hint/editor catalogs | `pkg/vm/compiler.go`, `pkg/vm/vm.go`, `pkg/vm/vm_test.go`, `pkg/vm/compiler_test.go`, `pkg/spl2/error_hints.go`, `web/src/editor/lynxflow-catalog.ts` |
