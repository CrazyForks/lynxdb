...
func TestQuery_BroadScopeLints(t *testing.T) {
	...
	// Pin to spl2 language: broad-scope lints are generated by the SPL2 query
	// analysis path (broadScopeLints). The LynxFlow path produces different lints.
	allSourceMeta := post(map[string]interface{}{"q": `FROM * | head 1`, "language": "spl2"})
	...
	broadSearchMeta := post(map[string]interface{}{"q": `FROM * | search request | head 1`, "language": "spl2"})
	...
	noLintMeta := post(map[string]interface{}{"q": `FROM * | search request | head 1`, "lint": false, "language": "spl2"})
	if _, ok := noLintMeta["lints"]; ok {
		t.Fatalf("meta.lints present despite lint=false: %#v", noLintMeta["lints"])
	}
...
func TestQuery_BroadScopeLintThresholdConfig(t *testing.T) {
	...
	// Pin to spl2: broad-scope lints are generated by the SPL2 analysis path.
	raw, _ := json.Marshal(map[string]interface{}{"q": `FROM * | head 1`, "language": "spl2"})
	resp, err := http.Post(fmt.Sprintf("http://%s/api/v1/query", srv.Addr()), "application/json", bytes.NewReader(raw))
	if err != nil {
		t.Fatalf("POST: %v", err)
	}
